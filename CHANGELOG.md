...
### Fixed
- **Cross-platform build** — `title_bar_style` and `traffic_light_position` on `WebviewWindowBuilder` are macOS-only; gate them behind `#[cfg(target_os = "macos")]` so Windows/Linux CI builds compile
